...
# 샘플 데이터
x = [[1], [2], [3], [4], [5]] # 데이터
y = [0, 1, 0, 1, 0] # 라벨링 1 = 긍정, 0 = 부정 -> 지도 학습

# train_test_split : 훈련 데이터와 테스트 데이터로 나누는데 사용하는 함수
# 훈련 데이터 : 모델을 학습시키는데 이용
# 테스트 데이터 : 학습된 모델의 성능을 평가하는데 사용
x_train, x_test, y_train, y_test = train_test_split(x, y, test_size = 0.2, random_state = 42)
# test_size = 0.2 : 데이터 상위 20%를 테스트 데이터로 사용하겠다. (나머지 80%는 훈련 데이터)
# random_state = 42 : 시드값 고정, 매번 실행할 때 동일한 순서로 동일한 값이 테스트 데이터에 포함됩니다. 

print('훈련 데이터 : ', x_train)
print('테스트 데이터 : ', x_test)

# 8 (훈련 데이터) : 2 (테스트 데이터) -> 일반적인 머신러닝 실험에서 모델의 성능을 평가하기 위한 데이터 분할 비용
...
# 텍스트 벡터화
# 텍스트를 벡터로 변환하는 이유 : 컴퓨터는 텍스트(문자열)를 직접 처리하지 못하기 때문에 숫자 벡터로 변환하는 과정이 필요함
...
# x 데이터(리뷰)와 y 데이터(라벨링 긍정인지 부정인지)의 정확도를 판단
# 저 리뷰가 정말 긍정인지, 부정인지 정확도 판단
