...
result = dpois(k, lanbda = 1)
cat('2시간 동안 새를 정확히 5마리 볼 확률 : ', round(result, 4), '\n')

# 문제
# 한 카페에서 시간당 평균 4잔의 커피를 판매합니다. 3시간 동안 *정확히 6잔의 커피가 판매될 확률을 구해보자.
lambda <- 12
l = 4 * 3
k = 6
...
# 문제
# 한 택배 회사에서 하루 평균 10건의 배송 지연이 발생됩니다. 일주일에 배송 지연이 3건 이하로 발생할 확률을 구해보자.
l = 10 * 7 # 주간 평균 지연 건수
k = 3
...
# 2. 하루동안 걸려 오는 전화 수
# 3. 원고 집필 시 원고지 한 장당 오타의 수
# 4. 금융상품 가입 상담 건수 10회 중 실제 가입이  이루어진 수
# 정답 : 4번 -> 이항분포, 고정된 시도 횟수가 있으며, 각 시도에서 성공/실패 두가지 존재