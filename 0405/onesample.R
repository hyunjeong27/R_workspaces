...
# 단일 집단의 표본 평균 vs 알려진 모집단 평균

# 제품 규격 준수 여부 확인
# 평균 18.5, 표준편차 2.3을 가진 50개의 랜덤 데이터
...
# p.value : p 값, 귀무가설이 참이라는 가정에 9따라 주어진 표본 데이터를 희소 또는 극한 값으로 얻을 확률
if(t_test_result$p.value < 0.05){
  print('금속 막대의 평균 길이가 150cm랑 다를 가능성이 높습니다.')
  print('귀무가설 기각') # 대립가설 채택
} else {
  print('금속 막대의 평균 길이는 알려진 것과 차이가 없다.')
  print('귀무가설 채택')
}
